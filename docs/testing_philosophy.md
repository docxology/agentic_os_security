...
- **Parity**: every `evidence.SOURCES` key has a matching `@...{key,` entry in `manuscript/references.bib` (65 + 6 = 71); every citation key in prose exists in the bib.
...
- **Integration**: `run_analysis` writes the 4 data artifacts + 6 figures; the validation report is all-green.
