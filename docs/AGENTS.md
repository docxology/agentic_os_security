...
1. **Accuracy over polish.** Every command, filename, and count in these docs must match the tree: scripts are `00_preflight.py`, `10_evaluation_analysis.py`, `z_generate_manuscript_variables.py`; data artifacts are the eleven analysis outputs in `output/data/`; figures are the eleven registered PNGs plus the cover graphical abstract.
...
3. **Constants are pinned.** 9 properties, 24 candidates, 8 scenarios, 7 trust domains, 9 controls, 9 configuration invariants, 11 registered figures (+ 1 cover), 18 manuscript sections, 216 matrix cells, 8 formal definitions + 2 remarks, 155 source-derived + 13 scholarly = 168 bib keys. Quote these numbers only from the registry modules or the brief — never from memory of a previous doc revision.
