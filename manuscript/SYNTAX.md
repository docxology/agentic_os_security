...
**Key parity is enforced by tests** (`tests/test_evidence.py`, `tests/test_manuscript_structure.py`): every citation key used in prose must exist in `references.bib`; the bibliography has 161 entries (155 primary sources + 6 scholarly anchors), extended in the v0.2.0 and v0.3.0 rounds (review date 2026-09-11).
...
Cover figure (no `{#fig:...}` label, not in the registry, not counted among the 9 registered figures): `output/figures/graphical_abstract.png`, generated by `generate_graphical_abstract()` — the paper's graphical abstract, written to the PDF cover and HTML page.
