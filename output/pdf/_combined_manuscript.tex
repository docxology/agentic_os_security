...
\makeatother
\vfill
\includegraphics[width=0.98\textwidth,height=0.6\textheight,keepaspectratio,alt={Graphical abstract: the two failure paths feed a nine-property evaluation of twenty-four operating systems, converging on the composition thesis — compartmentalization, reproducible operations, boot integrity, and capability-limited agents as one design target.}]{\detokenize{../figures/graphical_abstract.png}}
\vfill
\end{titlepage}
\thispagestyle{empty}
\tableofcontents
...
This revision (0.3.0) refreshes the evidence base through September 11,
...
Operating-System Boundary}\label{sec:introduction}
...
\section{Evaluation Framework: Nine Properties over Distribution
...
\section{Compartmentalization: Qubes OS Under Offensive-Agent
Load}\label{sec:qubes}
...
\section{Conventional Desktops: Hardening Candidates and Compatibility
Costs}\label{sec:desktops}
...
Disposable-Isolation Baseline}\label{sec:servers}
...
\section{Boundary Comparators: What Non-Linux Systems
...
determines what an attacker must defeat, and the set below is chosen so
that each control closes a failure the others leave open
\citep{cif_formal_2026}.
...
The formal grounding for these rules is \texorpdfstring{\ensuremath{\delta}}{delta}-bounded delegation: each hop
in an agent-to-agent chain is permitted to narrow the authority it
passes, never to widen it, so trust cannot amplify across a delegation
chain no matter how many intermediaries it traverses
\citep{cif_formal_2026}. That formal model is not left as a statement of
...
The 9 figures and the underlying analysis artifacts are reproducible
