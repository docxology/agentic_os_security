...
This revision (0.5.0) refreshes the evidence base through September 11,
...
Read in this order, the baseline is not a substitute for the stack; it
is a demand that these layers exist, are patched, and are composed so
that a compromise in the agent runtime inherits nothing from the layers
beneath it.
...
verification. The operator consuming agent output is, in this sense, in
the position of a system consuming the output of a possibly compromised
tool --- the same skepticism the review applies to build artifacts
applies to prose.
...
matter how many intermediaries it traverses \citep{cif_formal_2026}. The
bound of eq.~\ref{eq:delegation_bound} is not left as a statement of
intent --- its bounded-delegation and trust-boundedness properties have
been exercised computationally, with implementations validating the
...
\texttt{references.bib} --- 164 entries (155 source-derived plus 9
